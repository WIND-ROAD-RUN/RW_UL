...
namespace rw
{
	namespace hoem
	{
		...
		/*	•	单设备单连接，避免底层 PLC 的串行处理被多连接打爆或出现状态竞争。
			...
			*/
			/*是的，“优先级调度 + 串行化执行”在内部是异步的，但对调用方完全可以保持“同步语义”。做法是：把请求入队后在当前线程等待结果（future.get / 带超时的 wait_for），这样你发起读时仍然是“同步阻塞直到有结果或超时”，而高优先级让它能尽快插队到队头执行。
		怎么保证读时不被低优先级写长期阻塞
		•	不可中断正在进行的 Modbus 帧（协议限制），所以“最坏延迟 = 当前正在执行的一帧耗时 + 你的读帧耗时”。
		•	解决方案：
		1.	读请求标为高优先级，直接跳过低优先级队列（但不会中断在途帧）。
		2.	限制低优先级操作的单帧大小与发送速率，缩短可抢占点的间隔。
		3.	设置较小的 socket / 请求超时，失败快速返回，避免长时间卡住队列。
		4.	若厂商文档明确支持同一设备多客户端并发访问且互不影响，可考虑“只读独立连接”，写在主连接，读在只读连接上；否则仍以单连接为准。*/
