...
typedef enum _IMV_FG_EInterfaceType
{
	...
	typeInterfaceAll	= 0x00000000,			///< \~chinese 忽略接口类型		\~english ALL interfaces
...
enum EInterfaceMode
{
	...
	QUAD_BASE_MODE = 3,		///< \~chinese 四Base，同时支持4个相机		\~english Quad full
...
///信息：GIGE采集卡信息
/// \~english
///Information: GIGE Interface Information
typedef struct _IMV_GIGE_INTERFACE_INFO
{
	EInterfaceMode		nInterfaceMode;			///< \~chinese 采集卡模式					\~english Interface mode
	unsigned int        nPortInfo;				///< \~chinese 采集卡的Port信息			\~english Port Info
...
typedef struct _IMV_FG_INTERFACE_INFO
{
	...
	union
	{
		...
		IMV_CXP_INTERFACE_INFO		CxpInterfaceInfo;							///< \~chinese Cxp采集卡信息		\~english Cxp Capture Card info
		IMV_GIGE_INTERFACE_INFO		GigEInterfaceInfo;							///< \~chinese GigE采集卡信息		\~english GigE Capture Card info
...
typedef enum _IMV_FG_ECreateHandleMode
{
	...
	IMV_FG_MODE_BY_CAMERAKEY,				///< \~chinese 通过设备键"厂商:序列号"						\~english By device's key "vendor:serial number"	
	IMV_FG_MODE_BY_DEVICE_USERID,			///< \~chinese 通过设备自定义名								\~english By device userID
...
{
	uint64_t				blockId;				///< \~chinese 帧Id													\~english The block ID
	...
	unsigned int			paddingX;				///< \~chinese 图像paddingX(仅对GigE/Usb相机有效)					\~english The paddingX of image(GigE/Usb/PCIe Device only)
	unsigned int			paddingY;				///< \~chinese 图像paddingY(仅对GigE/Usb相机有效)					\~english The paddingY of image(GigE/Usb/PCIe Device only)
	unsigned int			recvFrameTime;			///< \~chinese 图像在网络传输所用的时间(单位:微秒,采集卡系列该值为0)	\~english The time taken for the image to be transmitted over the network(unit:us, The value is 0 for card camera)
...
typedef enum _IMV_FG_ESaveFileType
{
	...
	IMV_FG_TYPE_SAVE_TIFF_Extend	= 4,					///< \~chinese TIFF有效位拉伸至16Bit图像格式						  \~english TIFF Stretching effective bits to 16 bit image format
	IMV_FG_TYPE_SAVE_TIFF_RGB		= 5,					///< \~chinese TIFF(仅支持Bayer10\12\16)原图像格式					  \~english TIFF (support Bayer10\12\16 only) Original image format
	IMV_FG_TYPE_SAVE_TIFF_RGB_Extend = 6,					///< \~chinese TIFF(仅支持Bayer10\12\16)有效位拉伸至16Bit图像格式	  \~english TIFF (support Bayer10\12\16 only) Stretching effective bits to 16 bit image format
